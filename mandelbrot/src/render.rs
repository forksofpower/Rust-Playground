...
pub fn render_fractal(algorithm: &dyn FractalAlgorithm, config: &RenderConfig) -> Vec<u8> {
    ...
    let bands: Vec<(usize, &mut [u8])> =
        pixels.chunks_mut(config.dimensions.width).enumerate().collect();
...
fn render_band(
    algorithm: &dyn FractalAlgorithm, config: &RenderConfig, row: usize, band: &mut [u8],
) {
    let band_dims = Dimensions::new(config.dimensions.width, 1);

    // Calculate the complex region for this band
    let band_upper_left = config.region.pixel_to_point(config.dimensions, Pixel::new(0, row));
    let band_lower_right = config
        .region
        .pixel_to_point(config.dimensions, Pixel::new(config.dimensions.width, row + 1));
    ...
    for col in 0..config.dimensions.width {
        ...
        band[col] = match algorithm.calculate(point, config.limit) {
            None => 0,
            Some(count) => {
                let adjusted_count = if config.invert { config.limit - count } else { count };
